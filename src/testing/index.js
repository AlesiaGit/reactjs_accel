...
export function fakeCarMove(callback) {
  ...
  for(var i = 1; i < data.path.length; i++) {
    ((index) => {
      ...
      moveTimers[index] = setTimeout(() => callback(location), (data.path[index].timestamp - starttime)/5);
    })(i);
  }
}
...
export function fakeBumps(callback) {
  ...
  for(var i = 1; i < data.bumps.length; i++) {
    ((index) => {
      bumpsTimers[index] = setTimeout(() => callback(data.bumps[index]), (data.bumps[index].timestamp - starttime)/5);
    })(i);
  }
}
